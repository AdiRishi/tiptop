...
export async function getAssetByGuid(guid: string, versionSlug = 'latest', env: Env) {
  const db = getDrizzleClient(env);
  const asset = await db.select().from(assetTable).where(eq(assetTable.guid, guid)).get();
  if (!asset) {
    throw new AssetNotFoundException(`Asset with guid ${guid} not found`);
  }
  let assetVersion: AssetVersionSelectType | undefined;
  if (versionSlug === 'latest') {
    assetVersion = await db
